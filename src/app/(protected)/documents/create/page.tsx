...
import { generateDocumentDraftAction } from "@/lib/document-actions";
import { createEncryptedDocument, updateEncryptedDocument, getDecryptedDocument } from "@/lib/client-actions/documents";
...
function WizardContent() {
    ...
    // If editing existing doc, load and decrypt it
    useEffect(() => {
        if (docId && unions.length > 0) {
            setLoading(true);
            const union = unions.find(u => u.id === selectedUnionId);
            if (union?.encryptionKey) {
                getDecryptedDocument(docId, union.encryptionKey).then(({ document: doc }) => {
                    if (doc) {
                        setTitle(doc.title);
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const result = await generateDocumentDraftAction(templateId || "generic", {
                title,
                date,
                narrative,
                solution,
    ...
    const handleSave = async () => {
        ...
        const union = unions.find(u => u.id === selectedUnionId);
        if (!union?.encryptionKey) {
            toast.error("No encryption key available for this union");
            return;
        }
        setLoading(true);
        try {
