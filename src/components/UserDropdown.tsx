...
export function UserDropdown() {
    ...
    const { unions, activeUnion, setActiveUnion } = useUnion();
                ...
                <div className="absolute right-0 top-full mt-2 w-64 rounded-lg border bg-card p-2 shadow-lg animate-in fade-in zoom-in-95 duration-200 z-50">
                    ...
                    <div className="space-y-1">
                        {unions.map(u => (
                            <button
                                key={u.id}
                                onClick={() => { setActiveUnion(u); setIsOpen(false); }}
                                className="flex w-full items-center justify-between rounded-md px-2 py-1.5 text-sm hover:bg-accent hover:text-accent-foreground text-left"
                            >
                                <div className="flex items-center gap-2">
                                    <Building2 className="h-4 w-4" />
                                    <span className="truncate">{u.name}</span>
                                </div>
                                {activeUnion?.id === u.id && <Check className="h-3 w-3 text-primary" />}
                            </button>
                        ))}
                        <Link
                            href="/unions"
